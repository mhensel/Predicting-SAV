...
Env_Var_year.CBP_WQ %>% group_by(year)
...
write_csv(Env_Var_nosumy1sp.CBP_WQ.tss, "./Users/mhensel/Documents/_data/Ruppia/WaterQual_CBP2019.csv")

Env_Var_nosumy1sp.CBP_WQ.tss = read.csv("/Volumes/savshare2/Current Projects/Ruppia/Data/CBP WQ Station data/Env_Var_nosumy1sp.CBP_WQ.tss.csv")

Env_Var_nosumy1sp.CBP_WQ.tss %>% group_by(STATION) %>%
  summarise(missing.dat = across(c(Temp.spme, ChlA.spme, Sal.spme, Secc.spme, TN.spme, TP.spme), ~sum(is.na(.))))
